#ifndef L_NAMOA_DR_MVH_KDT_H
#define L_NAMOA_DR_MVH_KDT_H
...
class L_NAMOA_DR_MVH_KDT : public AbstractSolver {
public:
    std::pmr::unsynchronized_pool_resource node_pool;
    
    // The truncated (M-1) dimensional frontier stored as a Dynamic KD-Tree.
    std::vector<fast_mvh::DynamicFrontierKDTree<>> truncated_non_dominated_kdt;
    
    // The full M-dimensional Pareto set stored exactly as before for fallbacks.
    ...
    // Track KDT comparison operations
    uint64_t cmpchk = 0;
    uint64_t cmpupd = 0;
    ...
    std::string get_solver_name() override { return "L_NAMOA_DR_MVH_KDT"; }
    ...
    L_NAMOA_DR_MVH_KDT(const AdjacencyMatrix& adj_matrix, const EPS& eps);
