...
/*******************************************
 * Provenance: EMOA* reference implementation (see paper references). 
 * All Rights Reserved. 
 *******************************************/
...
#include "frontier_kdinc.hpp"
#include <cstdlib>
#include <map>
#include <string>

namespace { std::map<long, rzq::bench::OpStream> g_cap; bool g_cap_on = false; }
...
namespace basic { long long g_eps_dom_count = 0; long long g_coord_cmp_count = 0; }
...
void EMOA::InitHeu(long vd) {
  ...
  basic::g_eps_dom_count = 0; // reset dom check counter.
  return ;
};
...
int EMOA::Search(long vo, long vd, double time_limit) {
  ...
  g_cap_on = (std::getenv("EMOA_CAPTURE") != NULL);
  if (g_cap_on) g_cap.clear();
  ...
  _res.n_domCheck = basic::g_eps_dom_count;
  _res.rt_search = timer.GetDurationSecond();

  rzq::search::CaptureFlush(_graph->CostDim() - 1);
...
void EMOA::_InitFrontiers() {
  _alpha.resize(_graph->NumVertex()); // this requires the graph vertex are numbered from 0 to |V|.
  // Frontier backend is the AVL tree by default; EMOA_FRONTIER=kdinc opts into
  // the incremental k-d tree (projected dim = M-1). The baseline is untouched.
  const char* fe = std::getenv("EMOA_FRONTIER");
  size_t pdim = (_graph->CostDim() >= 1) ? _graph->CostDim() - 1 : 0;
  bool use_kdinc = (fe != NULL && std::string(fe) == "kdinc" && pdim >= 1);
  if (use_kdinc) {
    std::cout << "[INFO] EMOA frontier backend: kdinc (proj_dim=" << pdim << ")" << std::endl;
  }
  for (int idx = 0; idx < _alpha.size(); idx++){
    _alpha[idx] = use_kdinc ? static_cast<Frontier*>(new FrontierKdInc(pdim))
                            : new Frontier;
    _alpha[idx]->vid = idx;
  }
  return;
...
void CaptureOp(long vid, int op_type, const std::vector<double>& projected) {
  if (!g_cap_on) return;
  rzq::bench::Op o;
  o.type = (op_type == 0) ? rzq::bench::OP_CHECK : rzq::bench::OP_UPDATE;
  o.vec = projected;
  g_cap[vid].push_back(o);
}
void CaptureFlush(size_t dim) {
  if (!g_cap_on) return;
  const char* p = std::getenv("EMOA_CAPTURE");
  if (!p) return;
  // Each frontier is per-vertex, so write the busiest vertex's stream (the most
  // benchmark-relevant single-frontier workload) rather than mixing vertices.
  long best_vid = -1; size_t best_ops = 0;
  for (std::map<long, rzq::bench::OpStream>::const_iterator it = g_cap.begin();
       it != g_cap.end(); ++it) {
    if (it->second.size() > best_ops) { best_ops = it->second.size(); best_vid = it->first; }
  }
  rzq::bench::OpStream out;
  if (best_vid >= 0) out = g_cap[best_vid];
  std::cout << "[INFO] EMOA capture: " << g_cap.size() << " vertex frontiers, busiest vid="
            << best_vid << " with " << best_ops << " ops -> " << p << std::endl;
  rzq::bench::WriteStream(p, out, dim);
}
