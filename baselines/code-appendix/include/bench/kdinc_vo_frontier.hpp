...
// S3: incremental k-d frontier + online adaptive visit order. SOUND: only the
// order in which the two children are descended changes; the lo-corner prune
// and EpsDom test are identical to the baseline, so the Check result is
// unchanged. Each node learns which child more often supplies the dominator
// (lhits/rhits) and descends that one first to fire the early-exit sooner.
// New class so the baseline KDIncFrontier is untouched (spec isolation).
#ifndef RZQ_BENCH_KDINC_VO_FRONTIER_H_
#define RZQ_BENCH_KDINC_VO_FRONTIER_H_
...
class KDIncVOFrontier : public IFrontier {
public:
  explicit KDIncVOFrontier(size_t dim) : _dim(dim) {
    if (dim == 0) throw std::invalid_argument("KDIncVOFrontier: dim must be >= 1");
  }
  KDIncVOFrontier(const KDIncVOFrontier&) = delete;
  KDIncVOFrontier& operator=(const KDIncVOFrontier&) = delete;
  ~KDIncVOFrontier() { _free(_root); }
...
private:
  struct Node { int axis; bool dead; CostVec pt, lo, hi; Node* l; Node* r;
                long long lhits, rhits;
                Node() : axis(0), dead(false), l(NULL), r(NULL), lhits(0), rhits(0) {} };
  ...
  void _initLeaf(Node* n, const CostVec& gp, int axis) {
    ...
    n->l = NULL; n->r = NULL; n->lhits = 0; n->rhits = 0;
  }
  ...
  // Visit the historically-better child first; credit the child that produced
  // the dominator. Pure reordering => sound.
  bool _query(Node* n, const CostVec& g) {
    if (!n) return false;
    for (size_t d = 0; d < _dim; ++d) { if (n->lo[d] > g[d]) return false; }
    if (!n->dead && basic::EpsDom(n->pt, g)) return true;
    if (n->lhits >= n->rhits) {
      if (_query(n->l, g)) { ++n->lhits; return true; }
      if (_query(n->r, g)) { ++n->rhits; return true; }
    } else {
      if (_query(n->r, g)) { ++n->rhits; return true; }
      if (_query(n->l, g)) { ++n->lhits; return true; }
    }
